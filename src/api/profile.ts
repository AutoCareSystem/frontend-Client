...
export async function updateCutomerProfile(userId: string, dto: { name?: string; phone?: string; address?: string }) {
  const base = (import.meta as any).env?.VITE_API_BASE_URL || "http://localhost:5093";
  const url = `${base.replace(/\/+$/,'')}/api/Profile/customer/${encodeURIComponent(userId)}`;
  const token = localStorage.getItem("accessToken");
  const headers: any = { 'Content-Type': 'application/json' };
  if (token) headers.Authorization = `Bearer ${token}`;
  const res = await axios.put(url, dto, { headers, withCredentials: true });
